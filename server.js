...
- Include:
  - index.html with multiple sections (hero, about, products/services, Team, contact details, footer)
...
- Replace any contact form with a Team Section:
  - Include at least 5 team members
  - Each member must have a unique static Unsplash image URL (https://images.unsplash.com/...) with parameters ?w=800&h=600&fit=crop
  - Include alt text for each image
  - Display member name, role, and a short bio
  - Use Tailwind classes: object-cover rounded-lg mb-4 w-full h-64
  - Arrange members in a responsive grid (1 column mobile, 2 columns tablet, 3 columns desktop)
...
- Each section must have an ID matching the navbar link.
- Keep filenames consistent with references in the code
...
<code>
    ...
    {
      role: "user",
      content: `Intent: ${intent}\n\nGenerate the full code as files (index.html, style.css, script.js, server.js, package.json). Make sure the Team Section includes at least 5 members with unique images and proper responsive layout. Follow the output format exactly.`
    }
  ],
});
